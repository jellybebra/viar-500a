...
MIN_BRIGHT_DOCUMENT_AREA_RATIO = 0.06
...
def find_bright_document(image: cv2.Mat) -> np.ndarray:
    ...
    supplied mat is nearly black.  Otsu thresholding therefore finds even an A4
    sheet occupying only one side of the A3-sized camera field.
    """
    scaled = utils.resize_with_aspect_ratio(image=image, height=RESCALED_HEIGHT)
    ...
    frame_area = scaled.shape[0] * scaled.shape[1]
    candidates = []
    for contour in contours:
        area = cv2.contourArea(contour)
        ratio = area / frame_area
        if ratio < MIN_BRIGHT_DOCUMENT_AREA_RATIO or ratio > 0.95:
            continue
        ...
        if len(approximation) == 4:
            ...
            # Folded corners and shadows sometimes add extra points.  A minimum
            # area rectangle is a stable fallback for an otherwise large sheet.
            corners = cv2.boxPoints(cv2.minAreaRect(hull))

        rectangle_area = max(cv2.contourArea(corners.astype(np.float32)), 1)
...
def main(img: cv2.Mat) -> ScanResult:
    ...
    # First use the high-contrast paper/mat geometry of a document camera.
    bright_contour = find_bright_document(img)
    if bright_contour is not None:
        warped, bright_contour = extract_contour(
            image=img,
